...
// faster (cgo-based) bn256 package at ypenghui6/crypto/bn256.
package bn256ref // import "ypenghui6/crypto/bn256ref"
...
func (e *G1) FromX(x *big.Int) (*G1, bool) {
	...
	if y != nil {
		e.p = &curvePoint{x, y, big.NewInt(1), big.NewInt(1)}
		return e, true
	}
	return e, false
}
...
func (n *G1) Marshal() []byte {
	np := newCurvePoint(nil)
	np.Set(n.p)
	np.MakeAffine(nil)

	xBytes := new(big.Int).Mod(np.x, p).Bytes()
	yBytes := new(big.Int).Mod(np.y, p).Bytes()
...
func (n *G2) Marshal() []byte {
	np := newTwistPoint(nil)
	np.Set(n.p)
	np.MakeAffine(nil)

	xxBytes := new(big.Int).Mod(np.x.x, p).Bytes()
	xyBytes := new(big.Int).Mod(np.x.y, p).Bytes()
	yxBytes := new(big.Int).Mod(np.y.x, p).Bytes()
	yyBytes := new(big.Int).Mod(np.y.y, p).Bytes()
